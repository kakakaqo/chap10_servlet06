...
/**
 * DAO(Data Access Object) - 데이터베이스 관련 작업 전담 클래스 - JDBC 드라이버 로딩/커넥션 생성/쿼리문
 * 작성/쿼리문 실행
 */
...
public class MemberDAO {
	 ...
	 * maxwait : 풀에 커넥션이 없을 때 대기 가능한 최대 시간
	 * 			(-1 : 무한대기)
	 * [DataSource]
	 * 기본 생성자에서 데이터베이스 연결 관련 DataSource 객체 얻음
	 * context.xml에도 커넥션 관련 설정되어 있음.[옵션]
	...
	public MemberDAO() {
		try {
			 ...
			 * 환경정보 얻기
			 * "java:/comp/env" 매개변수는 환경관련 컨텍스트 개체를 찾는데
			 * 사용되는 자바의 표준 명명 규칙
			 */
			// [방법.1]
	...
	public ArrayList<MemberVO> listMembers() {
		...
		try {
			...
			// connectDB(); 데이터베이스 커넥션 생성 메소드 호출
